...
import {ButtonMenuItemOptions, ButtonMenuItemOptionsVerifiable, ButtonMenuSync} from '../buttonMenu';import { generateAIReply, addUserMessage } from '../../lib/openai';
...
export default class ChatInput {
  ...
  public constructPeerHelpers() {
    ...
    this.attachMenuButtons = [{
      icon: 'message',
      regularText: 'Reply with AI',
      onClick: async() => {
        const text = this.messageInput.innerText;
        if(!text) return;
        ...
        const aiReply = await generateAIReply(text, this.chat.peerId, agentType, customType);
        
        // Add the message to history
        addUserMessage(this.chat.peerId, text);
        if(aiReply) {
          this.messageInputField.setValueSilently(aiReply);
          this.onMessageInput();
        }
      }
    }, {
  ...
  // public async onMessageSent(clearInput = true, clearReply?: boolean) {
  ...
  //     // Handle auto AI reply for private chats
  //     if(!PEER_EXCEPTIONS.has(this.chat.type)) {
  //       console.log('Message sent, checking auto-reply conditions:', {
  //         autoReplyEnabled: appState.settings.autoAIReply,
  //         isUser: this.chat.peerId.isUser(),
  //         isBot: this.chat.isBot,
  //         isEditing: !!this.editMsgId
  //       });
  //     }

  //     if(appState.settings.autoAIReply && 
  //        this.chat.peerId.isUser() && 
  //        !this.chat.isBot && 
  //        !this.editMsgId) {
  //       console.log('Auto-reply conditions met, fetching context...');
  //       // Get recent messages for context
  //       const messages = await this.managers.appMessagesManager.getHistory({
  //         peerId: this.chat.peerId,
  //         limit: 5
  //       });
  ...
  //       const context = messages.messages.map((message) => {
  //         if(message._ === 'message' && message.message) {
  //           return `${message.pFlags.out ? 'Me' : 'Them'}: ${message.message}`;
  //         }
  //         return '';
  //       });
  ...
  //       console.log('Generating AI reply with context:', contextText);
        
  //       const aiReply = await generateAIReply(contextText);
  ...
  public async onMessageReceived(message: any) {
    if (!PEER_EXCEPTIONS.has(this.chat.type)) {
      console.log('Message received, checking auto-reply conditions:', {
        autoReplyEnabled: appState.settings.autoAIReply,
        isUser: this.chat.peerId.isUser(),
        isBot: this.chat.isBot,
      ...
      if (
        appState.settings.autoAIReply &&
        this.chat.peerId.isUser() &&
        !this.chat.isBot &&
        !message.pFlags.out // Check if the message is incoming
      ) {
        console.log('Auto-reply conditions met, fetching context...');
        // Get recent messages for context
        const messages = await this.managers.appMessagesManager.getHistory({
        ...
        const context = messages.messages.map((msg) => {
          if (msg._ === 'message' && msg.message) {
            return `${msg.pFlags.out ? 'Me' : 'Them'}: ${msg.message}`;
          }
          return '';
        });
        ...
        const aiReply = await generateAIReply(contextText, this.chat.peerId, agentType, customType);
        
        // Add the context message to history
        addUserMessage(this.chat.peerId, contextText);
        console.log('Got AI reply:', aiReply);
  ...
  public async sendMessage(force = false) {
    ...
    // this.onMessageSent();
    this.clearInput(true, true); // Add this line to clear the input
