...
export default class AppGeneralSettingsTab extends SliderSuperTabEventable {
  ...
  public init(p: ReturnType<typeof AppGeneralSettingsTab['getInitArgs']>) {
...

    {
      ...
      // Add auto reply agent selection
      // Create auto reply checkbox
      const autoReplyCheckbox = new CheckboxField({
        text: 'Enable TG Agents' as LangPackKey,
        name: 'auto-reply',
        stateKey: joinDeepPath('settings', 'autoAIReply'),
        listenerSetter: this.listenerSetter
      });

      const autoReplyRow = new Row({
        checkboxField: autoReplyCheckbox,
        listenerSetter: this.listenerSetter
      });

      // Create agent selection form
      const autoReplyAgentForm = document.createElement('form');
      autoReplyAgentForm.classList.add('auto-reply-agent-form');
      autoReplyAgentForm.style.marginLeft = '52px'; // Align with checkbox content
      autoReplyAgentForm.style.display = 'none'; // Hide by default

      const agentName = 'auto-reply-agent';
      const agentStateKey = joinDeepPath('settings', 'autoAIReplyAgent');

      const agents: [State['settings']['autoAIReplyAgent'], LangPackKey][] = [
        ['professional', 'Professional TG Agent' as LangPackKey],
        ['friendly', 'Friendly TG Agent' as LangPackKey],
        ['concise', 'Concise TG Agent' as LangPackKey],
        ['custom', 'Custom TG Agent' as LangPackKey]
      ];

      const agentRows = agents.map(([agent, langKey]) => {
        const row = new Row({
        ...
        // Add text input for custom agent type
        if(agent === 'custom') {
          const input = document.createElement('input');
          input.type = 'text';
          input.className = 'input-field';
          input.placeholder = 'Enter custom agent type...';
          input.style.marginLeft = '32px';
          input.style.marginTop = '8px';
          input.style.display = 'none';

          // Show/hide input based on radio selection
          this.listenerSetter.add(rootScope)('settings_updated', ({key, value}) => {
            if(key === agentStateKey) {
              input.style.display = value === 'custom' ? 'block' : 'none';
            }
          });

          // Save custom agent type
          input.addEventListener('change', () => {
            rootScope.managers.appStateManager.setByKey(
              joinDeepPath('settings', 'customAIReplyAgent'), 
              input.value
            );
          });

          // Set initial value if exists
          apiManagerProxy.getState().then((state: State) => {
            if(state.settings?.customAIReplyAgent) {
              input.value = state.settings.customAIReplyAgent;
            }
            input.style.display = state.settings?.autoAIReplyAgent === 'custom' ? 'block' : 'none';
          });

          row.container.append(input);
        }
      ...
      autoReplyAgentForm.append(...agentRows.map((row) => row.container));

      // Show/hide agent selection based on auto reply checkbox
      this.listenerSetter.add(rootScope)('settings_updated', ({key, value}) => {
        if(key === joinDeepPath('settings', 'autoAIReply')) {
          autoReplyAgentForm.style.display = value ? 'block' : 'none';
        }
      });

      // Set initial visibility
      apiManagerProxy.getState().then((state: State) => {
        autoReplyAgentForm.style.display = state.settings?.autoAIReply ? 'block' : 'none';
      });
